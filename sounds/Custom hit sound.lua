...
if #sound_names <= 0 then
    client.log('hitmarker.lua > there is no sounds "\\csgo\\sound"')
    return _exception()
end
...
client.set_event_callback("player_hurt", function(e)
    ...
    for k,v in pairs(sounds) do
        if k == ui_get(sound_ref) and sound_exists(v) and attacker == me and userid ~= me then
            local time = globals.tickinterval() * 1
            
            client.delay_call(time, client.exec, "play " .. v)
        end
    end
end)
